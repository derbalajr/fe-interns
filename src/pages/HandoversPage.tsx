...
function getUnitLabel(handover: Handover) {
  return handover.unit?.code
    ? `Unit ${handover.unit.code}`
    : `Unit #${handover.unit_id}`;
}

export function HandoversPage() {
  const { data, isLoading, isError, refetch } = useHandoversQuery();
  ...
  const thisMonthHandovers = handovers.filter((handover) => {
    const [year, month] = handover.handover_date.split("-").map(Number);
    const now = new Date();
      ...
      {isLoading ? (
        <div className="flex min-h-[320px] items-center justify-center">
          <div className="text-center">
            ...
            <p className="mt-4 text-sm text-[#626262]">Loading handovers…</p>
          </div>
        </div>
      ) : isError ? (
        <div className="flex min-h-[320px] items-center justify-center">
              ...
              <div className="min-h-[460px] rounded-2xl border border-black/10">
                    ...
                    {upcomingHandovers.length === 0 ? (
                      <div className="flex min-h-[380px] flex-col items-center justify-center text-center">
                        ...
                        <p className="mt-2 text-sm text-[#626262]">
                          When a reserved unit is ready, schedule its handover
                          here.
                        </p>
                      </div>
                    ) : (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <button
                                ...
                                onClick={() =>
                                  completeHandoverMutation.mutate(handover.id)
                                }
                ...
                {activeTab === "cancelled" && (
                  <div className="flex min-h-[430px] flex-col items-center justify-center p-6 text-center">
                    ...
                    <p className="mt-2 text-sm text-[#626262]">
                      Cancelled handovers are not currently supported by the
                      backend.
                    </p>
                  </div>
                )}
              ...
              <div className="min-h-[460px] rounded-2xl border border-black/10">
                <div className="p-6">
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" className="text-sm text-[#626262]">
                      View All →
                    </button>
                  </div>
                  ...
                  {completedHandovers.length === 0 ? (
                    ...
                    <div className="mt-6 space-y-5">
                      {completedHandovers.slice(0, 3).map((handover) => (
                        <div
                          key={handover.id}
  ...
  return (
    <div className="flex h-[124px] items-center rounded-2xl border border-[#ededed] bg-[#f8f8f8]/50 px-7 shadow-[0_2px_5px_rgba(0,0,0,0.12)]">
      <div className="flex h-[69px] w-[73px] shrink-0 items-center justify-center rounded-xl bg-[#4b4b4b]">
        <Icon size={42} strokeWidth={1.6} className="text-white" />
      </div>

      <div className="ml-[18px]">
        <p className="text-sm font-medium capitalize text-[#626262]">{label}</p>
